...
export class SQLManager<FieldsEnum extends string>
  ...
  async select(table: string, params: QueryParams<FieldsEnum>): Promise<any> {
    ...
    return this.sqlExecutor.queryRaw(query, values)
  }
  ...
  async count(table: string, params: CountParams<FieldsEnum>): Promise<number> {
    ...
    const result = await this.sqlExecutor.queryRaw(query, values)
  ...
  async batchInsert(
    ...
    for (let i = 0; i < data.length; i += batchSize) {
      ...
      try {
        const { query, values } = this.sqlBuilder.buildInsertQuery(table, {
          data: batchData,
        })
        await this.sqlExecutor.executeRaw(query, values)
      } catch (error) {
