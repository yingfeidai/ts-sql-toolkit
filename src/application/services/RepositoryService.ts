...
import { ISQLManager } from '../../domain/interfaces/ISQLManager'
...
export class Repository<DbModel, FieldsEnum extends string, DomainEntity>
  ...
  constructor(
    protected readonly sqlManager: ISQLManager<FieldsEnum>,
    protected readonly tableName: string,
    protected readonly mapper: IMapper<DbModel, DomainEntity>,
  ) {}
  ...
  async findMany(params: FindManyParams<FieldsEnum>): Promise<DomainEntity[]> {
    const results = await this.sqlManager.select(this.tableName, { ...params })
    return results.map((result: any) => this.mapper.toDomainEntity(result))
  }
